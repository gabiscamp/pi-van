...
class _StudentHomeTabState extends State<StudentHomeTab> {
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              const Text(
                'Para começar, entre na sala do seu motorista usando o código que ele compartilhou com você.',
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  _selectedStatus != null ? 'Chamada feita para hoje' : 'Marque sua chamada de hoje',
  ...
  Widget _buildInfoCards(dynamic user) {
    return Row(
      children: [
        Expanded(child: _miniCard(Icons.meeting_room_rounded, 'Sala', user.salaId != null ? 'Conectado' : 'Sem sala', AppTheme.primary)),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.how_to_vote_rounded, color: AppTheme.warning, size: 18),
              ),
              const SizedBox(width: 12),
              const Column(
                crossAxisAlignment: CrossAxisAlignment.start,
          ...
          Row(
            children: [
              Expanded(child: _attendanceOption('Ida e Volta', Icons.swap_horiz_rounded, AppTheme.primary, AttendanceStatus.vaiEVolta)),
              const SizedBox(width: 10),
              Expanded(child: _attendanceOption('Só Ida', Icons.arrow_forward_rounded, AppTheme.info, AttendanceStatus.soIda)),
            ],
          ),
          const SizedBox(height: 10),
          Row(
            children: [
              Expanded(child: _attendanceOption('Só Volta', Icons.arrow_back_rounded, AppTheme.accent, AttendanceStatus.soVolta)),
              const SizedBox(width: 10),
              Expanded(child: _attendanceOption('Não vou', Icons.close_rounded, AppTheme.error, AttendanceStatus.naoVai)),
            ],
          ),
        ],
      ),
  ...
  Widget _attendanceOption(String label, IconData icon, Color color, AttendanceStatus status) {
    final selected = _selectedStatus == status;
    return GestureDetector(
      onTap: () {
        setState(() => _selectedStatus = status);
        // TODO: Salvar no Firestore - salas/{salaId}/attendance/{date}/votes/{userId}
      },
      ...
      child: Column(
        children: [
          Row(
            children: [
              Container(
                width: 44, height: 44,
                decoration: BoxDecoration(
                  color: _liberado ? AppTheme.success.withOpacity(0.2) : AppTheme.grey100,
                  borderRadius: AppTheme.radiusMd,
                ),
                child: Icon(
                  _liberado ? Icons.check_circle_rounded : Icons.exit_to_app_rounded,
                  color: _liberado ? AppTheme.success : AppTheme.grey400,
                  size: 24,
                ),
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      _liberado ? 'O motorista foi notificado' : (_isAfter19h ? 'Toque para avisar o motorista' : 'Disponível a partir das 19h'),
          ...
          if (!_liberado) ...[
            ...
            AppButton(
              label: _isAfter19h ? 'Estou liberado' : 'Disponível às 19h',
              icon: Icons.exit_to_app_rounded,
              onPressed: _isAfter19h ? () {
                setState(() => _liberado = true);
                // TODO: Salvar liberado=true no Firestore + timestamp
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Motorista notificado!'), backgroundColor: AppTheme.success, behavior: SnackBarBehavior.floating),
                );
              } : null,
            ),
          ],
  ...
  Widget _buildCurrentStatus() {
    if (_selectedStatus == null) return const SizedBox();
    return Container(
      padding: const EdgeInsets.all(16),
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              'Sua chamada de hoje: ${_selectedStatus!.label}${_liberado ? ' • Liberado' : ''}',
