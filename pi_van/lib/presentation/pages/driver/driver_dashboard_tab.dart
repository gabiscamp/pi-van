...
class _DriverDashboardTabState extends State<DriverDashboardTab> {
  ...
  void initState() {
    super.initState();
    _currentSalaId = widget.viewModel.currentUser?.salaId;
    _startStreams();
    // Reage a trocas de sala ativa (e outras mudanças no usuário).
  ...
  /// Se a sala ativa mudou, reinicia os streams para a nova sala.
  void _onViewModelChanged() {
    final newSalaId = widget.viewModel.currentUser?.salaId;
    if (newSalaId != _currentSalaId) {
      _currentSalaId = newSalaId;
      _restartStreams();
    }
  }
  ...
  void _restartStreams() {
    ...
    _studentsSub = null;
    _attendanceSub = null;
    // Zera os contadores da sala anterior antes de carregar a nova.
    if (mounted) {
      setState(() {
        _totalStudents = 0;
        _confirmedToday = 0;
  ...
  void _startStreams() {
    ...
    final salaId = user!.salaId!;

    _studentsSub = repo.studentsStream(salaId).listen((students) {
      if (mounted) setState(() => _totalStudents = students.length);
    });

    _attendanceSub = repo.attendanceStream(salaId: salaId, date: _today).listen((votes) {
      if (!mounted) return;
