...
class _RegisterPageState extends State<RegisterPage> {
  ...
  @override
  void dispose() {
    for (var c in [_nameCtrl, _emailCtrl, _passwordCtrl, _confirmCtrl, _cepCtrl, _ruaCtrl, _numeroCtrl, _bairroCtrl, _cidadeCtrl, _ufCtrl, _complementoCtrl]) {
      c.dispose();
    }
    _pageCtrl.dispose();
    super.dispose();
  ...
  Future<void> _cadastrar() async {
    if (_cepCtrl.text.isEmpty || _ruaCtrl.text.isEmpty || _numeroCtrl.text.isEmpty || _bairroCtrl.text.isEmpty || _cidadeCtrl.text.isEmpty || _ufCtrl.text.isEmpty) {
      _showError('Preencha todos os campos de endereço');
      return;
    }
    ...
    try {
      await widget.viewModel.register(
        ...
        logradouro: _ruaCtrl.text, numero: _numeroCtrl.text,
        complemento: _complementoCtrl.text, bairro: _bairroCtrl.text,
        cep: _cepCtrl.text, localidade: _cidadeCtrl.text, uf: _ufCtrl.text,
        // TODO: geocodificar endereço com Nominatim para obter lat/lng
      );
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Cadastro realizado com sucesso!'), backgroundColor: AppTheme.success, behavior: SnackBarBehavior.floating),
      );
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text('Crie sua conta', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800, letterSpacing: -0.5)),
          const SizedBox(height: 8),
          const Text('Preencha seus dados para começar', style: TextStyle(color: AppTheme.grey500, fontSize: 14)),
          const SizedBox(height: 32),
          _label('Nome completo'),
          AppTextField(controller: _nameCtrl, label: 'Seu nome', prefixIcon: Icons.person_outline),
          const SizedBox(height: 16),
          _label('Email'),
          AppTextField(controller: _emailCtrl, label: 'seu.email@exemplo.com', keyboardType: TextInputType.emailAddress, prefixIcon: Icons.email_outlined),
          const SizedBox(height: 16),
          _label('Senha'),
          ...
          AppTextField(controller: _confirmCtrl, label: 'Confirme sua senha', obscureText: true, prefixIcon: Icons.lock_outlined),
          const SizedBox(height: 16),
          _label('Você é'),
